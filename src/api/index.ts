...
import { IPokemonDetailRequest, IPokemonSpeciesRequest, IRequest } from "../interfaces/IRequest";
...
export const getPokemonList = async (currentPage: number, limit: number, max: number): Promise<IPokemon[]> => {
    try {
...
export const getPokemonDetail = async (id: string): Promise<IPokemonDetail> => {
    try {
        ...
        const pokemonDetail: IPokemonDetail = {
            ...
            types: pokemon.types,
            flavor: species.flavor_text_entries[0]?.flavor_text.replace(/[\n\f]/g, " ") || 'N/A',
